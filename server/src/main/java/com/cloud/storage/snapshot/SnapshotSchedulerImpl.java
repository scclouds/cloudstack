...
public class SnapshotSchedulerImpl extends ManagerBase implements SnapshotScheduler {
    ...
    private void scheduleNextSnapshotJobsIfNecessary() {
        ...
        s_logger.info(String.format("Verifying the current state of [%s] snapshot schedules and scheduling next jobs, if necessary.", snapshotSchedules.size()));
    ...
    protected void scheduleNextSnapshotJobIfNecessary(SnapshotScheduleVO snapshotSchedule) {
        ...
        if (asyncJob == null) {
            s_logger.debug(String.format("The async job [%s] of snapshot schedule [%s] does not exist anymore. Considering it as finished and scheduling the next snapshot job.",
        ...
        if (JobInfo.Status.SUCCEEDED.equals(status)) {
            s_logger.debug(String.format("Last job of schedule [%s] succeeded; scheduling the next snapshot job.", snapshotSchedule));
        } else if (JobInfo.Status.FAILED.equals(status)) {
            s_logger.debug(String.format("Last job of schedule [%s] failed with [%s]; scheduling a new snapshot job.", snapshotSchedule, asyncJob.getResult()));
        } else {
            s_logger.debug(String.format("Schedule [%s] is still in progress, skipping next job scheduling.", snapshotSchedule));
            return;
        }
