...
public class IpAddressManagerImpl extends ManagerBase implements IpAddressManager, Configurable {
    ...
    protected boolean applyIpAssociations(IPAddressVO ipAddressVO, Network network, boolean success) {
        ...
        if (ipAddressVO.getState() == IpAddress.State.Allocating || ipAddressVO.getState() == IpAddress.State.Reserved) {
     ...
     */
    protected static void validateNetworkAndIpOwnership(Account owner, IPAddressVO ipToAssoc, Network network, DataCenter zone) {
        if (network.getAccountId() != owner.getId() && !network.getVpcId().equals(ipToAssoc.getVpcId()) && zone.getNetworkType() == NetworkType.Advanced
