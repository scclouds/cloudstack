...
public class QuotaBalanceDaoImpl extends GenericDaoBase<QuotaBalanceVO, Long> implements QuotaBalanceDao {
    private static final Logger s_logger = LogManager.getLogger(QuotaBalanceDaoImpl.class);
    ...
    public BigDecimal getLastQuotaBalance(final Long accountId, final Long domainId) {
        ...
        if (quotaBalance == null) {
            s_logger.info("There are no balance entries for account [{}] and domain [{}]. Considering only new added credits.", accountId, domainId);
        } else {
