...
public class StorageVMSnapshotStrategy extends DefaultVMSnapshotStrategy {
    ...
    public StrategyPriority canHandle(Long vmId, Long rootPoolId, boolean snapshotMemory) {
        ...
        if (CollectionUtils.isNotEmpty(vmSnapshotDao.findByVmAndByType(vmId, VMSnapshot.Type.DiskAndMemory))) {
            logger.debug(String.format("%s as it has VM snapshots with disk and memory.", cantHandleLog));
            return StrategyPriority.CANT_HANDLE;
        }

        if (!VirtualMachine.State.Running.equals(vm.getState())) {
            logger.debug(String.format("%s as the VM is not running.", cantHandleLog));
            return StrategyPriority.CANT_HANDLE;
        }
        ...
        logger.debug(String.format("%s as %s.", cantHandleLog, snapshotMemory ? "A VM snapshot with memory was requested" :
