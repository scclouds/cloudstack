...
public class KvmFileBasedStorageVmSnapshotStrategy extends StorageVMSnapshotStrategy {
        ...
        try {
            ...
            for (VolumeInfo volumeInfo : volumeInfoToSnapshotObjectMap.keySet()) {
                ...
                try {
                    ...
                    logger.error(String.format("Failed to change snapshot [%s] state due to [%s].", snapshot.getUuid(), e.getMessage()), e);
                }
    ...
    public boolean deleteVMSnapshot(VMSnapshot vmSnapshot) {
        logger.info(String.format("Starting VM snapshot delete process for snapshot [%s].", vmSnapshot.getUuid()));
        ...
        if (isCurrent && numberOfChildren == 0) {
            ...
            logger.debug(String.format("Deleting VM snapshot [%s] as no snapshots/volumes depend on it.", vmSnapshot.getUuid()));
    ...
    public boolean revertVMSnapshot(VMSnapshot vmSnapshot) {
        ...
        backupManager.prepareVmForSnapshotRevert(vmSnapshot);
    ...
    private void updateSizeIfNeeded(List<SnapshotDataStoreVO> volumeSnapshots, VolumeVO volumeVO, VolumeObjectTO volumeObjectTO) {
        ...
        if (volumeVO.getSize() == snapshotRef.getSize()) {
            logger.debug(String.format("No need to update the volume size and launch a resize event as the snapshot [%s] and volume [%s] size are equal.", snapshotRef.getSnapshotId(), volumeVO.getUuid()));
            return;
        }
    ...
    private void mergeOldSiblingWithOldParentIfOldParentIsDead(VMSnapshotVO oldParent, UserVmVO userVm, Long hostId, List<VolumeObjectTO> volumeTOs) {
        ...
        if (oldParent.getCurrent()) {
            ...
            if (oldSiblings.isEmpty()) {
                logger.warn(String.format("The old snapshot [%s] is dead, but it only had one child. This is an inconsistency and should be analysed/reported.", oldParent.getUuid()));
                return;
            }

            VMSnapshotVO oldSibling = oldSiblings.get(0);
            logger.debug(String.format("Merging VM snapshot [%s] with [%s] as the former was hidden and only the latter depends on it.", oldParent.getUuid(), oldSibling.getUuid()));
    ...
    public StrategyPriority canHandle(VMSnapshot vmSnapshot) {
        if (!VMSnapshot.State.Allocated.equals(vmSnapshot.getState())) {
            ...
            if (CollectionUtils.isEmpty(vmSnapshotDetails)) {
                logger.debug(String.format("KVM file based storage VM snapshot strategy cannot handle [%s] as it is not a KVM file based storage VM snapshot.",
                        vmSnapshot.getUuid()));
    ...
    public StrategyPriority canHandle(Long vmId, Long rootPoolId, boolean snapshotMemory) {
        ...
        if (snapshotMemory) {
            logger.debug(String.format("%s as a snapshot with memory was requested.", cantHandleLog));
            return StrategyPriority.CANT_HANDLE;
        }

        if (!Hypervisor.HypervisorType.KVM.equals(vm.getHypervisorType())) {
            logger.debug(String.format("%s as the hypervisor is not KVM.", cantHandleLog));
            return StrategyPriority.CANT_HANDLE;
        }

        if (CollectionUtils.isNotEmpty(vmSnapshotDao.findByVmAndByType(vmId, VMSnapshot.Type.DiskAndMemory))) {
            logger.debug(String.format("%s as there is already a VM snapshot with disk and memory.", cantHandleLog));
            return StrategyPriority.CANT_HANDLE;
        }
    ...
    private List<SnapshotVO> deleteSnapshot(VMSnapshotVO vmSnapshotVO, Long hostId) {
        ...
        if (answer == null || !answer.getResult()) {
            logger.error(String.format("Failed to delete VM snapshot [%s] due to %s.", vmSnapshotVO.getUuid(), answer != null ? answer.getDetails() : "Communication failure"));
            throw new CloudRuntimeException(String.format("Failed to delete VM snapshot [%s].", vmSnapshotVO.getUuid()));
        }

        logger.debug(String.format("Updating metadata of VM snapshot [%s].", vmSnapshotVO.getUuid()));
    ...
    private List<SnapshotVO> mergeSnapshots(VMSnapshotVO vmSnapshotVO, VMSnapshotVO childSnapshot, UserVmVO userVm, Long hostId) {
        ...
        logger.debug(String.format("Updating metadata of VM snapshot [%s] and its child [%s].", vmSnapshotVO.getUuid(), childSnapshot.getUuid()));
        List<SnapshotVO> snapshotVOList = new ArrayList<>();
    ...
    private List<SnapshotVO> mergeCurrentDeltaOnSnapshot(VMSnapshotVO vmSnapshotVo, UserVmVO userVmVO, Long hostId, List<VolumeObjectTO> volumeObjectTOS) {
        logger.debug(String.format("Merging VM snapshot [%s] with the current volume delta.", vmSnapshotVo.getUuid()));
    ...
    protected VMSnapshot takeVmSnapshotInternal(VMSnapshot vmSnapshot, Map<VolumeInfo, SnapshotObject> volumeInfoToSnapshotObjectMap) throws NoTransitionException {
        ...
        logger.info(String.format("Starting disk-only VM snapshot process for VM [%s].", userVm.getUuid()));
        ...
        logger.info(String.format("Sending disk-only VM snapshot creation of VM Snapshot [%s] command for host [%s].", vmSnapshot.getUuid(), hostId));
        ...
        logger.error(String.format("Disk-only VM snapshot for VM [%s] failed%s.", userVm.getUuid(), answer != null ? " due to" + answer.getDetails() : ""));
    ...
    private VMSnapshot processCreateVmSnapshotAnswer(VMSnapshot vmSnapshot, Map<VolumeInfo, SnapshotObject> volumeInfoToSnapshotObjectMap, CreateDiskOnlyVmSnapshotAnswer answer, UserVm userVm, VMSnapshotVO vmSnapshotVO, long virtualSize, VMSnapshotVO parentSnapshotVo) throws NoTransitionException {
        logger.debug(String.format("Processing CreateDiskOnlyVMSnapshotCommand answer for disk-only VM snapshot [%s].", vmSnapshot.getUuid()));
        ...
        for (VolumeInfo volumeInfo : volumeInfoToSnapshotObjectMap.keySet()) {
            ...
            logger.debug(String.format("Updating metadata for volume [%s] and its corresponding snapshot [%s].", volumeVO, snapshot.getSnapshotVO()));
    ...
    private long createVolumeSnapshotMetadataAndCalculateVirtualSize(VMSnapshot vmSnapshot, Map<VolumeInfo, SnapshotObject> volumeInfoToSnapshotObjectMap, List<VolumeObjectTO> volumeTOs) throws NoTransitionException {
        ...
        for (VolumeObjectTO volumeObjectTO : volumeTOs) {
            ...
            logger.debug(String.format("Creating snapshot metadata [%s] as part of the disk-only snapshot process for VM [%s].", snapshot, volumeObjectTO.getVmName()));
    ...
    private List<DeltaMergeTreeTO> generateDeltaMergeTrees(VMSnapshotVO parent, VMSnapshotVO child, List<VMSnapshotVO> grandChildren, boolean stoppedVm) throws NoSuchElementException {
        logger.debug(String.format("Generating list of Snapshot Merge Trees for the merge process of VM Snapshot [%s].", parent.getUuid()));
    ...
    private void removeCurrentBackingChainSnapshotFromVmSnapshotList(List<VMSnapshotVO> vmSnapshotList, UserVm userVm) {
        ...
        for (VMSnapshotVO vmSnapshotVO : vmSnapshotList) {
            if (currentBranch.stream().anyMatch(currentBranchSnap -> Objects.equals(currentBranchSnap.getId(), vmSnapshotVO.getId()))) {
                logger.trace(String.format("Removing snapshot [%s] from the list of VM snapshots of VM [%s] being rebased.", vmSnapshotVO.getUuid(), userVm.getUuid()));
