...
public class DefaultVMSnapshotStrategy extends ManagerBase implements VMSnapshotStrategy {
    ...
    public StrategyPriority canHandle(Long vmId, Long rootPoolId, boolean snapshotMemory) {
        ...
        if (State.Running.equals(vm.getState()) && (!snapshotMemory || vmHasKvmDiskOnlySnapshot(vm))) {
            logger.debug(String.format("Default VM snapshot cannot handle VM snapshot for [%s] as it is running and its memory will not be affected.", vm));
        ...
        for (VolumeVO volume : volumes) {
            if (volume.getFormat() != ImageFormat.QCOW2) {
                logger.debug(String.format("Default VM snapshot cannot handle VM snapshot for [%s] as it has a volume [%s] that is not in the QCOW2 format.", vm, volume));
                return StrategyPriority.CANT_HANDLE;
            }
        }
        return StrategyPriority.DEFAULT;
