...
public class UsageManagerImpl extends ManagerBase implements UsageManager, Runnable {
    ...
    private boolean parseHelperTables(AccountVO account, Date currentStartDate, Date currentEndDate) {
        ...
        if (!parsed) {
            logger.debug(String.format("Backup object usage not parsed for account [%s].", account));
        }
    ...
    protected void handleBackupObjectEvent(UsageEventVO event) {
        ...
        if (EventTypes.EVENT_VM_BACKUP_CREATE.equals(eventType)) {
            ...
            logger.trace(String.format("Created usage backup object [%s].", usageBackupObject));
            ...
            logger.trace(String.format("Deleted usage backup object with ID [%s].", backupId));
        } else {
            logger.error(String.format("Unknown event type [%s] in backup object event parser. Skipping it.", eventType));
        }
    }
