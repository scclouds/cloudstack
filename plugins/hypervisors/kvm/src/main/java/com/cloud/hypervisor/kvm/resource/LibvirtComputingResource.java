...
public class LibvirtComputingResource extends ServerResourceBase implements ServerResource, VirtualRouterDeployer {
    ...
    private final Map<String, LibvirtExtendedVmStatsEntry> _vmStats = new ConcurrentHashMap<>();
    ...
    public PowerState getVmState(final Connect conn, final String vmName) {
        ...
        while (retry-- > 0) {
            try {
                ...
                s_logger.error(String.format("Can't get state for VM [%s] (retry=%s) due to [%s].", vmName, retry, e.getMessage()), e);
            } finally {
                try {
                    if (vms != null) {
                        vms.free();
    ...
    public VmStatsEntry getVmStat(final Connect conn, final String vmName) throws LibvirtException {
        ...
        try {
            ...
            LibvirtExtendedVmStatsEntry oldStats = _vmStats.get(vmName);
    ...
    /**
     * Passes a VM's current CPU statistics into the provided VmStats.
     * @param dm domain of the VM.
    ...
    /**
     * Passes a VM's current network statistics into the provided VmStats.
     * @param dm domain of the VM.
    ...
    /**
     * Passes a VM's current disk statistics into the provided VmStats.
     * @param dm domain of the VM.
