...
public class QuotaResponseBuilderImpl implements QuotaResponseBuilder {
    ...
    protected Map<String, BigDecimal> createQuotaDateMap(List<QuotaUsageJoinVO> usageRecords, QuotaStatementCmd cmd, BigDecimal quotaUsed) {
        if (quotaUsed.equals(BigDecimal.ZERO)) {
            logger.debug("Not aggregating quota by time because item has not used any quota in the period.");
            return null;
        }
        if (cmd.getAggregationInterval() == ApiConstants.AggregationInterval.NONE) {
            logger.debug("Not aggregating quota by time because AggregationInterval is set to None.");
            return null;
        }
        ...
        logger.debug("Creating a time sorted map with [{}] aggregation for the used quota.", cmd.getAggregationInterval());
    ...
    protected void setStatementItemResources(QuotaStatementItemResponse statementItem, int usageType, List<QuotaUsageJoinVO> quotaUsageRecords, QuotaStatementCmd cmd) {
        if (!cmd.isShowResources()) {
            logger.debug("Not creating resources statement for the item because isShowResources is set to false.");
            return;
        }
    ...
    protected void findQuotaUsagesAndDetailsByTariffAndAccount(Long accountId, Long domainId, String tariffName, Integer usageType, Date startDate, Date endDate, List<QuotaUsageJoinVO> tariffUsageRecords, List<QuotaUsageDetailVO> tariffUsageDetails) {
        ...
        logger.debug("Found [{}] tariffs named [{}].", idsOfTariffsThatHaveTheSameName.size(), tariffName);
        for (Long id : idsOfTariffsThatHaveTheSameName) {
            tariffUsageDetails.addAll(quotaUsageDetailDao.listQuotaUsageDetails(accountId, domainId, usageType, id, startDate, endDate));
    ...
    protected List<QuotaTariffStatementItemResponse> createQuotaTariffStatementItemList(QuotaTariffStatementCmd cmd) {
        ...
        logger.debug("Found a total of [{}] quota tariffs.", quotaTariffs.size());
        ...
        for (QuotaTariffVO quotaTariff : quotaTariffs) {
            if (processedTariffs.contains(quotaTariff.getName())) {
                logger.debug("Skipping tariff [{}] since a statement for a tariff with the same name was already generated.", quotaTariff);
                continue;
            }
            processedTariffs.add(quotaTariff.getName());
            logger.debug("Generating statement for tariff [{}].", quotaTariff);
            ...
            if (tariffStatementItemResponse.getQuotaUsed().compareTo(BigDecimal.ZERO) > 0) {
                logger.debug("Generated statement for tariff [{}] has a quota usage [{}] greater than zero; therefore, it will be added to the response.", quotaTariff, tariffStatementItemResponse.getQuotaUsed());
                quotaTariffStatementItemResponseList.add(tariffStatementItemResponse);
            } else {
                logger.debug("Generated statement for tariff [{}] has a quota usage equal to zero; therefore, it will not be added to the response.", quotaTariff);
            }
        }
    ...
    public QuotaTariffStatementResponse listQuotaTariffUsage(QuotaTariffStatementCmd cmd) {
        ...
        logger.debug("The total quota usage for account [{}] in domain [{}] of tariff [{}] and usage type [{}] between [{}] and [{}] was [{}].", accountId, domainId, tariffName, usageType, startDate, endDate, totalQuotaUsed);
    ...
    protected void setTariffStatementItemResources(QuotaTariffStatementItemResponse statementItem, int usageType, List<QuotaUsageDetailVO> quotaUsageDetailList, List<QuotaUsageJoinVO> quotaUsageRecords) {
        ...
        logger.info("Calculating how much quota each resource consumed with tariff [{}].", tariffAsString);
        ...
        for (QuotaUsageDetailVO quotaUsageDetail : quotaUsageDetailList) {
            ...
            if (correspondingQuotaUsageRecord == null) {
                logger.debug("Could not find a quota usage entry corresponding to quota usage detail [{}]; therefore, this detail will not be considered in tariff [{}]'s resources.", quotaUsageDetail, tariffAsString);
                continue;
            }

            Long resourceId = getResourceIdByUsageType(correspondingQuotaUsageRecord, usageType);
            if (resourceId == null) {
                logger.debug("Could not find a resource for quota usage detail [{}]; therefore, this detail will not be considered in tariff [{}]'s resources.", quotaUsageDetail, tariffAsString);
                continue;
            }

            BigDecimal quotaUsedByResource = ObjectUtils.defaultIfNull(quotaUsageValuesAggregatedByResourceId.get(resourceId), BigDecimal.ZERO);
            quotaUsageValuesAggregatedByResourceId.put(resourceId, quotaUsedByResource.add(quotaUsageDetail.getQuotaUsed()));
        }

        logger.info("Adding the calculated quota usage of [{}] resources into tariff [{}]'s response item.", quotaUsageValuesAggregatedByResourceId.size(), tariffAsString);
        List<QuotaStatementItemResourceResponse> itemDetails = new ArrayList<>();
    ...
    protected QuotaTariffStatementResponse createQuotaTariffStatementResponse(QuotaTariffStatementCmd cmd, List<QuotaTariffStatementItemResponse> quotaTariffStatementItemResponseList, BigDecimal totalQuotaUsed) {
        ...
        Long domainId = ObjectUtils.defaultIfNull(cmd.getDomainId(), account.getDomainId());
        DomainVO domain = domainDao.findByIdIncludingRemoved(domainId);
        if (domain != null) {
    ...
    public QuotaUsageDetailsResponse listUsageDetails(QuotaStatementDetailsCmd cmd) {
        ...
        for (QuotaUsageJoinVO quotaUsageJoin : quotaUsageJoinList) {
            ...
            logger.debug("Found [{}] quota usage details associated to the quota usage [{}] of resource [{}] and type [{}] between [{}] and [{}].", quotaUsageDetailsList.size(), quotaUsageJoin, resourceUuid, usageType, startDate, endDate);
    ...
    protected Pair<Long, String> retrieveResourceIdAndNamePair(String resourceUuid, Integer usageType) {
        ...
        switch (usageType) {
            ...
            case QuotaTypes.RUNNING_VM:
                ...
                if (vmInstance != null) {
                    logger.debug("Found VM [{}] with ID [{}] and of type [{}].", vmInstance, resourceUuid, usageType);
                    validateCallerAccessToResource(vmInstance);
            ...
            case QuotaTypes.VM_DISK_IO_WRITE:
                ...
                if (volume != null) {
                    logger.debug("Found volume [{}] with ID [{}] and of type [{}].", volume, resourceUuid, usageType);
                    validateCallerAccessToResource(volume);
            ...
            case QuotaTypes.SNAPSHOT:
                ...
                if (snapshot != null) {
                    logger.debug("Found snapshot [{}] with ID [{}] and of type [{}].", snapshot, resourceUuid, usageType);
                    validateCallerAccessToResource(snapshot);
            ...
            case QuotaTypes.NETWORK_BYTES_RECEIVED:
                ...
                if (network != null) {
                    logger.debug("Found network [{}] with ID [{}] and of type [{}].", network, resourceUuid, usageType);
                    validateCallerAccessToResource(network);
            ...
            case QuotaTypes.ISO:
                ...
                if (image != null) {
                    logger.debug("Found image [{}] with ID [{}] and of type [{}].", image, resourceUuid, usageType);
                    validateCallerAccessToResource(image);
            ...
            case QuotaTypes.BACKUP:
                ...
                if (backupOffering != null) {
                    logger.debug("Found backup offering [{}] with ID [{}] and of type [{}].", backupOffering, resourceUuid, usageType);
                    validateCallerAccessToResource(backupOffering);
            ...
            case QuotaTypes.NETWORK_OFFERING:
                ...
                if (networkOffering != null) {
                    logger.debug("Found network offering [{}] with ID [{}] and of type [{}].", networkOffering, resourceUuid, usageType);
                    validateCallerAccessToResource(networkOffering);
    ...
    protected void validateCallerAccessToResource(Object resource) {
        Account caller = CallContext.current().getCallingAccount();
        _accountMgr.validateAccountHasAccessToResource(caller, SecurityChecker.AccessType.UseEntry, resource);
    }
    ...
    protected QuotaUsageDetailsItemResponse createQuotaUsageDetailsItemResponse(QuotaUsageDetailVO quotaUsageDetail, List<QuotaTariffVO> quotaTariffs, Date startDate, Date endDate) {
        logger.trace("Creating quota usage details item associated to quota usage detail [{}].", quotaUsageDetail);
    ...
    protected QuotaUsageDetailsResponse createQuotaUsageDetailsResponse(String resourceName, String resourceUuid, Integer usageType,
        ...
        logger.trace("Creating quota usage details list response associated to the resource of name [{}] and UUID [{}], with an usage type of [{}], [{}] quota" +
                " usage details items, and a total quota used of [{}].", resourceName, resourceUuid, usageType, quotaUsageDetailsItems.size(), totalQuotaUsed);
    ...
    public List<ResourcesQuotingResultResponse> quoteResources(String resourcesToQuoteAsJson) {
        logger.trace(String.format("Parsing JSON [%s] to a list of [%s].", resourcesToQuoteAsJson, ResourcesToQuoteVO.class.getName()));
        ...
        try {
            ...
            logger.error(String.format("Could not parse the JSON passed as parameter [%s] due to [%s].", resourcesToQuoteAsJson, e.getMessage()), e);
        ...
        logger.info("Validating quotings fields.");
    ...
    protected Set<Integer> validateResourcesToQuoteFieldsAndReturnUsageTypes(List<ResourcesToQuoteVO> resourcesToQuote) throws InvalidParameterValueException {
        ...
        logger.debug("Validating usage types set in the quotings.");
        ...
        for (int index = 0; index < resourcesToQuote.size(); index++) {
            ...
            logger.debug(String.format("Adding [%s] to the set of usage types that are being quoted.", usageType));
            usageTypes.put(usageType, resourceToQuote.getUsageType());

            addIdToResourceToQuoteIfNotSet(index, resourceToQuote);
        }

        logger.debug(String.format("The following usage types were identified for quoting and will be used to retrieve the quota tariffs: %s.", usageTypes.values()));
    ...
    protected void addIdToResourceToQuoteIfNotSet(int index, ResourcesToQuoteVO resourceToQuote) {
        ...
        logger.debug(String.format("Quoting at index [%s] does not have an ID. We set [%s] as the ID for this quote.", index, index));
    ...
    protected int validateResourceToQuoteUsageTypeAndReturnsItsId(int index, String usageType) throws InvalidParameterValueException {
        logger.debug(String.format("Validating quoting field \"usageType\" at index [%s].", index));
    ...
    protected void validateCallerAccessToAccountsAndDomainsPassedAsParameterInQuotingMetadata(List<ResourcesToQuoteVO> resourcesToQuote) throws PermissionDeniedException {
        ...
        logger.debug(String.format("Validating caller [%s] access to the accounts and domains passed as parameter in the quotings.", caller));

        for (int index = 0; index < resourcesToQuote.size(); index++) {
            PresetVariables metadata = resourcesToQuote.get(index).getMetadata();

            if (metadata == null) {
                logger.trace(String.format("Quoting at index [%s] does not have metadata. Skipping account and domain access check.", index));
                continue;
            }
        ...
        logger.debug(String.format("Caller [%s] has access to the accounts and domains passed as parameter in the quoting metadata.", caller));
    }
    ...
    protected void validateCallerAccessToAccountSetInQuotingMetadata(Account caller, PresetVariables metadata, int index) throws PermissionDeniedException {
        ...
        if (accountId == null) {
            logger.debug(String.format("Quoting metadata at index [%s] does not have an account. Skipping account access check.", index));
            return;
        }

        logger.trace(String.format("Searching account with UUID [%s], informed in quoting metadata at index [%s].", accountId, index));
        Account account = _accountDao.findByUuidIncludingRemoved(accountId);

        if (account == null) {
            logger.debug(String.format("Account with UUID [%s], informed in quoting metadata at index [%s], does not exist. Skipping account access check.", accountId, index));
            return;
        }

        logger.trace(String.format("Checking caller [%s] access to account [%s], informed in quoting metadata at index [%s].", caller, account, index));
        _accountMgr.checkAccess(caller, null, false, account);
        logger.trace(String.format("Caller [%s] has access to account [%s], informed in quoting metadata at index [%s].", caller, account, index));
    }
    ...
    protected void validateCallerAccessToDomainSetInQuotingMetadata(Account caller, PresetVariables metadata, int index) throws PermissionDeniedException {
        ...
        if (domainId == null) {
            logger.debug(String.format("Quoting metadata at index [%s] does not have a domain. Skipping domain access check.", index));
            return;
        }

        logger.trace(String.format("Searching domain with UUID [%s], informed in quoting metadata at index [%s].", domainId, index));
        DomainVO domain = domainDao.findByUuidIncludingRemoved(domainId);

        if (domain == null) {
            logger.debug(String.format("Domain with UUID [%s], informed in quoting metadata at index [%s], does not exist. Skipping domain access check.", domainId, index));
            return;
        }

        logger.trace(String.format("Checking caller [%s] access to domain [%s], informed in quoting metadata at index [%s].", caller, domain, index));
        _accountMgr.checkAccess(caller, domain);
        logger.trace(String.format("Caller [%s] has access to domain [%s], informed in quoting metadata at index [%s].", caller, domain, index));
    }
    ...
    protected List<ResourcesQuotingResultResponse> quoteResources(List<ResourcesToQuoteVO> resourcesToQuote, Set<Integer> usageTypes) {
        ...
        logger.info("Starting resources quoting.");
        ...
        if (logger.isTraceEnabled()) {
            logger.trace(String.format("Starting resources [%s] quoting with the tariffs [%s].", resourcesToQuote, validTariffsForQuoting));
        } else {
            logger.debug("Starting resources quoting with the filtered tariffs.");
        }
        ...
        try (JsInterpreter jsInterpreter = new JsInterpreter(QuotaConfig.QuotaActivationRuleTimeout.value(), QuotaConfig.QuotaActivationRuleTimeout.key())) {
            ...
            logger.error(String.format("An exception was thrown while quoting [%s]. Exception: %s.", resourceToQuote, e.getMessage()));
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(String.format("The quotings [%s] resulted in the total value [%s].", resourcesToQuote, finalQuotingValue));
        } else {
            logger.info(String.format("The quotings resulted in the total value [%s].", finalQuotingValue));
        }
    ...
    protected ResourcesQuotingResultResponse quoteResource(Map<Integer, List<QuotaTariffVO>> validTariffsForQuoting, ResourcesToQuoteVO resourceToQuote,
        ...
        logger.debug(String.format("Starting quoting of resource [%s].", resourceToQuote));
        ...
        if (tariffs == null || volumeToQuote == 0) {
            logger.debug(String.format("Either there are no tariffs [%s] for usage type [%s] or the volume to quote [%s] is 0. Setting quoting [%s] result as 0.",
        ...
        logger.debug(String.format("The quoting [%s] resulted in the value [%s].", quoteId, resourceQuotingResult.getQuote()));
        return resourceQuotingResult;
    }
